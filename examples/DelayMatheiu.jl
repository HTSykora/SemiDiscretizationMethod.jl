...
T = 5π #Principle period of the system (sin(t)=sin(t+P)) 
mathieu_lddep = createMathieuProblem(3.0, 0.2, -0.15, 0.1, T=T); # LDDE problem for Hayes equation
method = SemiDiscretization(0, 0.25) # 3rd order semi discretization with Δt=0.1
...
@time fp_LR_SP = fixPointOfMapping(mapping_LR_SP);
...
scatter(stab_border_points..., xlim=(-1.0, 5), ylim=(-2.0, 1.5),
